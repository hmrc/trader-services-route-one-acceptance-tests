...
port_mappings="$port_mappings,11000->11000,9557->9557,9553->9553,9554->9554,9556->9556,9597->9597"
...
if [ -z "${1}" ]; then
  ...
  BROWSER="artefacts.tax.service.gov.uk/chrome-with-rinetd:94-0.7.0"
elif [ "${1}" = "remote-firefox" ]; then
  BROWSER="artefacts.tax.service.gov.uk/firefox-with-rinetd:92-0.6.0"
fi
...
sbt \
  -Dbrowser=${1} \
  -Denvironment=local \
  clean 'testOnly uk.gov.hmrc.test.ui.cucumber.runner.Runner'